...
abstract class CActiveRecord extends CModel
{
	...
	public function __get($name)
	{
		...
		{
			$lie=$this->linkedRelations();
			$ret=$this->getLinkedAttributes($name,$lie);
			if(isset($ret))
				return $ret;
			else
	...
	**/
	...
	public function linkedRelations(){
		if(!isset($this->lrel)) 
			$this->lrel = $this->setLinkedRelations(); 
		return $this->lrel;
	}
	...
	public function attributeDatatypeRelation($attr)
	{
		...
		$datat = $this->getDataTypeLinkedRelation($attr,$this->linkedRelations());
		if(isset($datat))
			return $datat;
		return null;
	}
	
	public function getDataTypeLinkedRelation($attr,$linkedRel)
	{
		foreach($linkedRel as $key=>$values)
		{
			...
			{
				$ra = $this->getDataTypeLinkedRelation($attr,array($value['attribute']=>$values['linked']));
				if(isset($ra))
					return $ra;
			}
		}
	...
	public function getLinkedAttributes($attr, $linkedRel=array())
	{
		
		foreach($linkedRel as $key=>$value)
		{
			if(is_array($value) && isset($value['attribute']) && $value['attribute']==$attr)
				return $value['value']==null ? '':$value['value'];
			else if(is_array($value) && isset($value['linked']))
			{
				$ra = $this->getLinkedAttributes($attr,array($value['attribute']=>$value['linked']));
				if(isset($ra))
					return $ra;
			}
